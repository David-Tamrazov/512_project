...
public class LockManager
{
                ...
                synchronized (this.lockTable) {
                    ...
                    if (!bConflict) {
                        ...
                        if (bConvert.get(0) == true) {
                            ...
                            // get whatever lock this txn has on this resource
                            DataObj lock = getLock(xid, strData);

                            if (lock != null) {
                                
                                // redundant lock conversion
                                if (lock.getLockType() == lockType) {
                                    throw new RedundantLockRequestException("Redundant lock conversion request.");
                                }

                                TrxnObj to = new TrxnObj(lock.getXId(), lock.getDataName(), lock.getLockType());
                                this.lockTable.remove(to);
                                this.lockTable.remove(lock);

                                // if this txn is going to convert to a read lock, notify all other txn's waiting for a read lock that they can now acquire one 
                                if (lockType == LockManager.READ) {
                                    notifyWaitingForRead(lock);
                                }
    ...
    private DataObj getLock(int xid, String strData) {
        ...
        for (int i = 0; i < size; i++) {
            ...
            // if this lock object is for the resource and transaction whose lock we're trying to remove...
            if (xid == dataObj2.getXId() && strData == dataObj2.getStrData()) {    
        ...
        // no lock found- return an empty object and check for it wherever the method is called 
        return dataObj2;
    ...
    private void notifyWaitingForRead(DataObj dataObj) {
        ...
        synchronized (this.waitTable) {
            ...
            for (int j = 0; j < waitSize; j++) {
                ...
                if (waitObj.getLockType() == LockManager.READ) {
                    // remove interrupted thread from waitTable.
                    this.waitTable.remove(waitObj);    
